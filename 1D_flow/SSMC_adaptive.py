...
def mcmc_steps(norm_old, jr):
    ...
    for it in range(mc_steps):
        ...
        for com in range(10):
            norm_new[com] = norm_old[com] + np.random.normal(0,jr[com],1) 
...
#%% SMC for posterior
nrp = 10 # nr of particles
cess_op = 0.95 # optimal CESS
qq = 0.05 # quantile for adaptive thresholds
mc_steps = 20 # MH steps per intermediate step
...
for ppp in range(nr_run):
    ...
    while alpha2[t]<threshold3:
        ...
        if np.mod(ppp,2) == 0 and alpha1[t] == 1:
            alpha2[t+1] = np.quantile(conc[t,:],qq)
        if np.mod(ppp,2) == 1:
            alpha2[t+1] = alpha2_s[t+1]
            
        plt.plot(ld_curr.T)
        plt.plot(ld_true.T, linestyle='dashed', color='black')
        plt.ylim([-20,-2])
        plt.show()
        ...
        if alpha2[t] >= threshold3 and alpha2[t-1] < threshold3:
            alpha2[t] = threshold3
            I = conc[t-1,conc[t-1,:] >= threshold3] 
            probb3[ppp] = len(I)/nrp*np.prod(pr[0:t])
    ...
    mdic1 = {'probb1':probb1, 'probb2':probb2, 'probb3':probb3, 'alpha1': alpha1, 'alpha2': alpha2, 'sigma': sigma, 'conc': conc, 'pr': pr}
    name1 = 'Diff_SMC2_ada40.mat'
